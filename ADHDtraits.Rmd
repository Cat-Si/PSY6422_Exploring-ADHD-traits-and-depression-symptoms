...
##Data wrangling 
*** 
Before any visualisations could be conducted on the data, a large amount of data warngling was required to get it to a useable form. 
...
```
```{r}
themeadj = theme_light(base_size = 15) + theme(legend.background = element_rect(fill = "transparent", colour =)) + theme(plot.title = element_text(size = 12, face = "bold"))
headingadj = theme(plot.caption = element_text(hjust = 0, size = 8))
```
...
p <- ggplot(df, aes(asrs, cesd,)) 
  p <- p +  geom_point(mapping = aes(size = ders, color = ders)) +
       geom_smooth(method = "lm", se = F, colour = "cyan4") +
